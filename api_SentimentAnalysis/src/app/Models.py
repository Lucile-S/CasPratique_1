...
if __name__=="__main__":
    ...
    output_filename = 'results_glove.csv'
    saved_model_suffix_filename = 'Best_model.joblib'
    ROC_plot_filename = 'ROC_curves_glove.png'
    ...
    for tokenizer_name, tokenizer in tokenizers.items():
        ...
        for vectorizer_name, vectorizer in Vectorizers.items():    
            ...
            classifiers = {
                'Naive Bayes Multinomial':  NaiveBayes, # When using Spacy Word2vec Negative values in data passed to MultinomialNB (input X)
                'SVM': SVM, 
            ...
            for classifier_name, classifier in classifiers.items():
                if classifier_name == "Naive Bayes Multinomial" and vectorizer_name == "Spacy W2V": # else error => Negative value in data passed to MultinomialNB 
                    continue
    ...
    # -- Save df_results
    Makedir(output_DIR)
    df_results.sort_values(by=['eval_F1score'], ascending=False).to_csv(os.path.join(output_DIR, output_filename), index=False, header = True, sep = ';', encoding = 'utf-8') 
    ...
    print(f'\n {Best_model}')
    saveModel(Best_model,  os.path.join(model_DIR, Best_model_name.replace(' ','').replace('+','-') +'_'+ saved_model_suffix_filename))
